 ...
 * TEST PATH:<testing/model/trade_tdd>
 * 
 * @author Youcef Adoum
 * @package RHfiles
 * @version 1.0
...
class Trade_tdd extends CI_Controller
{
    ...
    public function index(){
        ...
        $test8=$this->checkingProposalsStatus($test5_proposal_id,"pending approval");
        $expected_result=TRUE; 
        echo $this->unit->run($test8,$expected_result,$test_name,$notes="New proposals default status is pending approval");

        $test_name="Checking a proposal can be accepted";
        $test9=$this->checkingProposalCanBeAccepted($test5_proposal_id,$this->proposed_dates);
        $expected_result=TRUE;
        echo $this->unit->run($test9,$expected_result,$test_name,$notes="True if category id is returned");

        $test_name="Checking proposal is in progress";
        $test10=$this->checkingProposalsStatus($test5_proposal_id,"inprogress");
        $expected_result=TRUE; 
        echo $this->unit->run($test10,$expected_result,$test_name,$notes="New proposals default status is pending approval");

        $test_name="Checking a proposal can be marked as complete and a trade created";
        $test11=$this->checkingProposalCanBeCompleted($test5_proposal_id);
        $expected_result=TRUE; 
        echo $this->unit->run($test11,$expected_result,$test_name,$notes="Asserts the Category name in db matches the testcase");

        $test_name="Checking the users scores have been updated ";
        $test12=$this->checkingUsersScoresHaveBeenUpdated($test1_user_a_id,$test3_user_b_id);
        $expected_result=TRUE; 
        echo $this->unit->run($test12,$expected_result,$test_name,$notes="Asserts the Category name in db matches the testcase");

        $test_name="Deleting the trade";
        $test3=$this->cleanupCreatedTrade($test6_proposal);
        $expected_result=TRUE; 
        ...
        $test8=$this->cleanupProposalCreated($test5_proposal_id);
        $expected_result=TRUE; 
    ...
    private function checkingProposalsStatus($proposal_id,$status)
    {
        return $proposal_status=$this->trade_model->read_trade_status($proposal_id,TRUE,$status);   
    }

    private function checkingProposalCanBeAccepted($test6_proposal,$accepted_date)
    {
        return $this->proposal_model->update_proposal_inprogress($test6_proposal,$accepted_date);
    }
    private function checkingProposalCanBeCompleted($proposal_id)
    {
        return $this->trade_model->create_trade_and_mark_proposal_complete($proposal_id);
    }

    private function checkingUsersScoresHaveBeenUpdated($user_a_id,$user_b_id)
    {
        return ($this->profile_model->read_score($user_a_id)==$this->skill_b_details['value'] && $this->profile_model->read_score($user_b_id)==$this->skill_a_details['value']);
    }

    private function cleanupCreatedTrade($proposal)
    {
        return $this->trade_model->delete_trade($proposal['initial_skill_id']);
    }

    private function cleanupProposalCreated($proposal_id)
    {
